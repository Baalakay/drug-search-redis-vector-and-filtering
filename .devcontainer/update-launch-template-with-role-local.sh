...
if [ ! -f "launch-template-userdata-with-ssh.sh" ]; then
    echo "❌ Missing: launch-template-userdata-with-ssh.sh"
    echo "Please copy it with: scp ec2-devcontainer:/workspaces/DAW/.devcontainer/launch-template-userdata-with-ssh.sh ."
    exit 1
fi
...
USER_DATA_B64=$(base64 -w 0 "launch-template-userdata-with-ssh.sh")
